...
def test_create_and_list_module(client, auth_headers):
    response = client.post(
        "/api/modules",
        json={"name": "Fitness", "schema_definition": {"fields": ["reps"]}},
...
def test_duplicate_module_name_for_same_user_rejected(client, auth_headers):
    client.post("/api/modules", json={"name": "Fitness"}, headers=auth_headers)
    response = client.post("/api/modules", json={"name": "Fitness"}, headers=auth_headers)
...
def test_same_module_name_allowed_for_different_users(client, auth_headers):
    ...
    response = client.post("/api/modules", json={"name": "Fitness"}, headers=other_headers)
    assert response.status_code == 201


def test_user_cannot_see_or_modify_other_users_module(client, auth_headers):
    created = client.post("/api/modules", json={"name": "Fitness"}, headers=auth_headers).json()
...
def test_filter_modules_by_is_active(client, auth_headers):
    client.post("/api/modules", json={"name": "Active One"}, headers=auth_headers)
    inactive = client.post(
        "/api/modules", json={"name": "Inactive One"}, headers=auth_headers
    ).json()
    client.put(
...
def test_delete_module_with_entries_blocked(client, auth_headers):
    module = client.post("/api/modules", json={"name": "Fitness"}, headers=auth_headers).json()
    client.post(
        "/api/entries",
