...
interface Section {
  name: string;
  label: string;
  /** Overrides the selector's shared `accent` for just this pill (see theme/moduleAccent.ts). */
      ...
      {visibleSections.length > 1 && (
        <View className="flex-row gap-2 border-b border-border bg-surface p-3">
          {visibleSections.map((s) => {
            const pillClasses = moduleClassNames[s.accent ?? accent];
            return (
              <Pressable
                key={s.name}
                onPress={() => setSelected(s.name)}
                className={`flex-1 items-center rounded-md py-2 ${
                  activeName === s.name ? pillClasses.bg : 'bg-surface-raised'
                }`}>
                <Text
